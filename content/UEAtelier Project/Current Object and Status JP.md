...
v0.35 開発では UE 5.7 と UE 5.8 を優先し、UE 5.6 は当初、移行期の maintenance canary としてのみ残します。主な製品焦点は RAG の reliability と retrieval quality、つまり test isolation、empty/stale index recovery、schema parity、engine-version-aware retrieval、説明可能な ranking、rank-aware eval gate です。これは開発方針であり、現在の公開版が UE 5.8 をサポート済みという意味ではありません。詳細は [[v0.35 Development Plan JP]]。
