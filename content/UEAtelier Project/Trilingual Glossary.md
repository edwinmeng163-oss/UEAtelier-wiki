...
| official MCP preview | 公式 MCP プレビュー | 官方 MCP 预览 | `v0.33.0-preview` — EXPERIMENTAL UE 5.8 official-MCP validation build from branch `experiment/v0.33-ue58-validation`; published on GitHub as a Pre-release; never merges to main as-is. The supported public line remains v0.34.0 (UE 5.6/5.7). |
| ToolRegistry | ToolRegistry | ToolRegistry | Explicit reviewed registry for tool metadata and policy: canonical `Tools/UnrealMcpToolRegistry/tools.json` (190 entries @ v0.34.0) with byte-equivalent mirror `Plugins/UnrealMcp/Resources/ToolRegistry/tools.json`; checked by `python3 Tools/validate_tool_registry.py`. |
...
| legacy hidden | legacy hidden | legacy hidden / 旧兼容隐藏 | Registry `exposure: legacy_hidden` for compatibility tools hidden from AI-facing `tools/list` — 12 of 190 entries @ v0.34.0 (e.g. `unreal.spawn_actor`, `unreal.mcp_apply_scaffold`, 5 legacy gameplay scaffolds), so `tools/list` exposes 178. |
| public mainline | 公開 mainline | 公开主线 | Supported `main` release line, currently v0.34.0 for UE 5.6/5.7. |
