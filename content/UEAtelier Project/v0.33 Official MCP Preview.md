...
This note corrects an important version split: `v0.33.0-preview` is not the normal UEAtelier MCP structure on `main`. It is an experimental UE 5.8 official-MCP validation branch. At v0.35.0 it remains evidence-only: v0.35 forward-ported only the small JSON-object compatibility shim and the unreachable-code cleanup, not the official-MCP stack; the dual-variant / official-MCP integration is a later v0.35 batch.
...
| Supported public line after v0.32.2 | `v0.35.0` for UE 5.7 / UE 5.8 (v0.34.0 was the last UE 5.6-packaged line) |
